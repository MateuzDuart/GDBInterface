...
class GDBManager:
    ...
    @staticmethod
    def create_session(gdb_path="D:\\android-NDK\\prebuilt\\windows-x86_64\\bin\\gdb.exe"):
...
class GDB:
        ...
        for line in disassembly:
            ...
            if match:
                ...
                parsed_output.append(
                    [is_current_line, is_breakpoint, address, offset, instruction, arguments]
                )
    ...
    def parse_memory(output):
        memory_mappings = []
        memory_pattern = re.compile(r'~"\\t(0x[0-9a-f]+)\s+(0x[0-9a-f]+)\s+(0x[0-9a-f]+)\s+(0x[0-9a-f]+)\s+(.*)\\n"')
        for line in output:
